...
This module is intentionally import-safe: if `bpy` is not available at
import time we provide no-op register/unregister functions so test
collection and CI don't fail. When running inside Blender the real
...
def _register_scene_properties() -> None:
    ...
    try:
        ...
        # Import-time no-op when bpy is not available (CI/tests)
        return
