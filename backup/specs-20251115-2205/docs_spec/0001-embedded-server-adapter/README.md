...
Implementation
--------------
- New module: `src/blender_mcp/servers/embedded_adapter.py` exposing a tiny API:
...
- The adapter does not embed a server runtime itself; callers provide the
  command to run. Tests and callers should patch `blender_mcp.servers.embedded_adapter`.
